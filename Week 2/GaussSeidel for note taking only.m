...
% Divides each row by the diagonal value
for ii = 1:N
    for jj = 1:M
...
while (max(abs(x_new-x_bak) > toler)) && (counter < 100000)
    counter = counter + 1;
    x_bak = x_old;      % to back up x_old. % so we can compare to it later
    for ii = 1:N        %% this for loop is to calculate each of x_new(ii)
        ...
        % Then we need to add on the original one 
        % NOTE that we divided each row by the diagonal,
        %   so we should have divided d by the real one as well
